...
Based on the summary of amniota dataset, some of the values don't make sense. Thus, I think NAs are represented by -999, -999.0, -999.00, -999.000,-999.0000, -999.00000 and -999.000000 for numeric variables.
...
NAs are represented by NA. NA might also represent a choice of binary variables.  
...
There is no NA in amniota. This values does not make sense since the NAs in amniota are represented by -999
...
## [1] 170566
```
There are 170566 NA's in amphibio.
...
amniota%>%
  group_by(class)%>%
  summarize(na_count = sum(is.na(egg_mass_g))) %>%
  arrange(class)
```
...
These missing data underline potential biases where species have been studied and documented.
...
Yes, NA may represent not-applicable for these binary variables.
