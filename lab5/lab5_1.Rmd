...
```{r}
filter(fish, lakeid == "AL" & length > 350)#meet both conidions that  in AL and Length >350
```

Notice that the `|` operator generates a different result.
```{r}
filter(fish, lakeid == "AL" | length > 350) # either or 
```
...
```{r}
new_mammals <- select(mammals,"genus", "mass", "gestation","newborn")
```

```{r}
primate <- filter(new_mammals, genus %in% c("Lophocebus" , "Erythrocebus"  ,"Macaca")) ## within is %in%
