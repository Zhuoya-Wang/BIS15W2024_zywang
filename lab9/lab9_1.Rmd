...
```{r}
relig_income <- read_csv("data/relig_income.csv");relig_income

```

2. Why are these data untidy?  

Because the col names are obesrvations

3. Use `pivot_longer()` to make the data tidy.  
```{r}
relig_income %>% 
  pivot_longer(-religion, #patient will not move
               names_to = "income", #make a new column called "drug"
               values_to="total" #values moved to a new column called 
               )
```
...
```{r}
plant_data <- read_csv("data/plant_data.csv");plant_data
```
