...
```{r}
levels(hot_springs$scientist) ## show the levels of scientist factor 
```
...
fish <- read_csv("/Users/wzy0329/Desktop/GitHub/BIS15W2024_zywang/lab3/data/Gaeta_etal_CLC_data.csv")
```
...
```{r}
#another way to show the structure
glimpse(fish)
```
...
```{r}
litle_fish <- filter(fish, length <=100);litle_fish
```
