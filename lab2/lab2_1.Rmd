...
else <- 12 ## provide specific name of object
```
...
```{r}
ari_mean <- sum(venom_GT, chiron, veyron)/3

```

3. Use the function `mean()` to calculate the mean top speed for the cars.
```{r}
mean_cars <- mean(c(venom_GT, chiron, veyron))
```
...
```{r}
mean_vector <- mean(new_vector);mean_vector
```

3. How do you interpret this result? What does this mean about NAs?  

The result is NA, which means R can not compute the mean of the new_vector. 
