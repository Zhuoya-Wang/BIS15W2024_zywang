...
```{r}
days_of_the_week[3]## the third element of that vector
```
...
```{r}
earnings = colSums(all_harry_potter_matrix)
```


2. Add this information to the data matrix. Hint: you are adding a row, not a column.
```{r}
new_matrix <- rbind(all_harry_potter_matrix, earnings );new_matrix
```
