...
fish %>% 
  ...
  select(fish_id, length, length_mm) ###creating a new column based on the present data called length_mm & select several columns
```
...
```{r}
mammals %>%
  mutate_all(tolower) ## taking all of the observations to lowercase
```

Using the across function we can specify individual columns.
```{r}
mammals %>% 
  mutate(across(c("order", "family"), tolower)) # to certain coloums to make them lowercase
```
...
```{r}
mammals %>% 
  select(genus, species, newborn) %>% #colum of interest
  mutate(newborn_new = ifelse(newborn == -999.00, NA, newborn))%>% #make a new colum if newborn = -999, make it to NA, others just keep it as newborn
  arrange(newborn) #
```
