...
## Load the data #citing data is important
...
```{r}
fish <- readr::read_csv("data/Gaeta_etal_CLC_data.csv")## reader is a part of tydyverse
```
...
```{r}
select(fish, fish_id:length) ## select from the start to end col
```
...
```{r}
glimpse(mammals)
summary(mammals) # mass can not be -999 suspect NAs were replaced by -999
```

2. Are there any NAs? Are you sure? Try taking an average of `max. life` as a test.  
```{r}
#anyNA() can be used to check if there any NAs
sum(is.na(mammals))
...
```{r}
mammals <- rename(mammals, genus = "Genus", afr = "AFR", maxlife = "max. life", littersyears = "litters/year")
```
...
```{r}
library(janitor)
#cleannames()## remove those problem directly
```
