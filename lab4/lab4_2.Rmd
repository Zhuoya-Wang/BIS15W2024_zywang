...
## `filter()` 
Unlike `select()`, `filter()` allows us to extract data that meet specific criteria within a variable. Let's say that we are interested only in the fish that occur in lake "AL". We can use `filter()` to extract these observations(rows)
```{r}
...
```{r}
filter(fish, length %in% c(167, 175)) # choose the observation with length = 167 and 175
```

Alternatively, you can use `between` if you are looking for a range of specific values.  
```{r}
filter(fish, between(scalelength, 2.5, 2.55))#filter the observation with scalelength in between (2.5 2.55)
```

You can also extract observations "near" a certain value but you need to specify a tolerance.  
```{r}
filter(fish, near(radii_length_mm, 2, tol = 0.2))# tol = tolerance
```
